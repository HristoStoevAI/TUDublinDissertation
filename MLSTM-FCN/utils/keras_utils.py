...
def train_model(model:Model, dataset_id, dataset_prefix, dataset_fold_id=None, epochs=50, batch_size=128, val_subset=None,
    ...
    if compile_model:
        model.compile(optimizer=optm, loss='categorical_crossentropy', metrics=['accuracy',f1_m,
    ...
    modelHistory=model.fit(X_train, y_train, batch_size=batch_size, epochs=epochs, callbacks=callback_list,
    ...
    with open(resultsFilename, "a") as text_file:
        ...
        print(f"Mean Epoch Time: {float(sum(modelTimes[0:int(convergenceEpochs)]))/int(convergenceEpochs)}\n", file=text_file)
...
def evaluate_model(model:Model, dataset_id, dataset_prefix, dataset_fold_id=None, batch_size=128, test_data_subset=None,
    ...
    optm = Adam(lr=1e-3)
    model.compile(optimizer=optm, loss='categorical_crossentropy', metrics=['accuracy',f1_m,precision_m, 
                                                                            recall_m, tp_m, fp_m, 
                                                                            tn_m, fn_m])
    ...
    print("\nEvaluating : ")
    loss, accuracy, f1_score, precision, recall, tp, fp, tn, fn = model.evaluate(X_test, y_test, batch_size=batch_size)
